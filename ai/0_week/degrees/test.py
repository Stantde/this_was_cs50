...
def shortest_path(source, target):
    ...
    path to return from function call.
    frontier
    nodes
    ...
    core loop:
        ...
        examine node in frontier
            if goal in frontier, (under the microscope) stop
        check for neighbors of node
            if neighbor already explored, don't add it to the list of nodes to explore
        make node of each neighbor
        end loop

    after finding goal, do this to trace back
    ...
    while not frontier.empty():
        ...
        potential_nodes = neighbors_for_person(currentNode.state)
        for neighbor in potential_nodes:
            if neighbor[1] not in explored:
                frontier.add(Node(neighbor[1], currentNode, neighbor[0]))
